...
-- PostgreSQL
-- Yukio hernandez canek gabriel favbor de revisar esto y corregir si esta mal 
-- ============================================================
...
-- ============================================================
-- Esto me base en las notas de campos y un poquito de claude 
...
DECLARE
    ...
    v_nombre        VARCHAR(40);
BEGIN
    -- (a) Verificar que el producto exista y este disponible
    ...
    -- (b) Calcular el subtotal del detalle (cantidad x precio unitario)
    NEW.precio_platillo := NEW.cantidad * v_precio;

    -- (c) Recalcular el total de la orden sumando todos los subtotales
    UPDATE ORDEN
    SET total = (
...
-- recalcula el total de la orden
-- ============================================================
-- Tampoco es dificil osea es como programrar basicamentew 

CREATE OR REPLACE FUNCTION fn_eliminar_detalle_orden()
RETURNS TRIGGER AS $$
BEGIN
    -- Recalcular total de la orden sin el registro eliminado
    UPDATE ORDEN
    SET total = (
...
-- -> total de ORDEN se recalcula sin ese producto
-- ------------------------------------------------------------
